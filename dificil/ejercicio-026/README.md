# Ejercicio 026 - MongoDB Dificil: arquitectura 3D
...
Una academia tecnica esta construyendo una plataforma de datos para analizar arquitectura 3D. El equipo necesita guardar informacion flexible, consultarla rapido y documentar por que MongoDB es una buena opcion para este caso.

En este ejercicio trabajaras con la coleccion `products`. La idea no es copiar comandos sin pensar: debes decidir que forma tendran los documentos, que campos son importantes y como validar que tus consultas responden la pregunta del negocio.

## Objetivo

Definir indices compuestos usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
1. Lee el contexto y define que datos necesita guardar la coleccion `products`.
...
```javascript
db.products.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.products.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
