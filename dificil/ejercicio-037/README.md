# Ejercicio 037 - MongoDB Dificil: autos de lujo
...
En este ejercicio trabajaras con la coleccion `movies`. La idea no es copiar comandos sin pensar: debes decidir que forma tendran los documentos, que campos son importantes y como validar que tus consultas responden la pregunta del negocio.

## Objetivo

Analizar cardinalidad y crecimiento usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
1. Lee el contexto y define que datos necesita guardar la coleccion `movies`.
...
```javascript
db.movies.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.movies.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
