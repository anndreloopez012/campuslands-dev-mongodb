# Ejercicio 019 - MongoDB Dificil: ropa urbana
...
En este ejercicio trabajaras con la coleccion `orders`. La idea no es copiar comandos sin pensar: debes decidir que forma tendran los documentos, que campos son importantes y como validar que tus consultas responden la pregunta del negocio.
...
1. Lee el contexto y define que datos necesita guardar la coleccion `orders`.
...
```javascript
db.orders.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.orders.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
