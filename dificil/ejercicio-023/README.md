# Ejercicio 023 - MongoDB Dificil: tatuajes
...
Una academia tecnica esta construyendo una plataforma de datos para analizar tatuajes. El equipo necesita guardar informacion flexible, consultarla rapido y documentar por que MongoDB es una buena opcion para este caso.
...
Disenar pipelines multi-etapa usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.tracks.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.tracks.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
