# Ejercicio 043 - MongoDB Dificil: peliculas de miedo
...
```javascript
db.fighters.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.fighters.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
