# Ejercicio 029 - MongoDB Dificil: esports shooter
...
En este ejercicio trabajaras con la coleccion `teams`. La idea no es copiar comandos sin pensar: debes decidir que forma tendran los documentos, que campos son importantes y como validar que tus consultas responden la pregunta del negocio.
...
1. Lee el contexto y define que datos necesita guardar la coleccion `teams`.
...
```javascript
db.teams.find({ categoria: 'principal' }).explain('executionStats')
```

```javascript
db.teams.aggregate([{ $match: { temporada: '2026' } }, { $lookup: { from: 'auditorias', localField: '_id', foreignField: 'refId', as: 'auditoria' } }, { $project: { nombre: 1, scoreFinal: { $multiply: ['$score', '$factor'] }, auditoria: 1 } }])
```
