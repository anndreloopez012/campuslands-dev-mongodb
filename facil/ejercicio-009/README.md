# Ejercicio 009 - MongoDB Facil: autos de lujo
...
```javascript
use campus_autos_de_lujo
```

```javascript
db.sessions.insertOne({ nombre: 'demo-9', puntos: 63, activo: true })
```

```javascript
db.sessions.find({ puntos: { $gte: 45 } }, { nombre: 1, puntos: 1, _id: 0 })
```
