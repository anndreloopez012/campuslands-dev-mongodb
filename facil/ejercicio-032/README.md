# Ejercicio 032 - MongoDB Facil: battle royale
...
Insertar documentos con estructura consistente usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
2. Crea entre 5 y 8 documentos de prueba con datos coherentes.
...
```javascript
use campus_battle_royale
```

```javascript
db.matches.insertOne({ nombre: 'demo-32', puntos: 224, activo: true })
```

```javascript
db.matches.find({ puntos: { $gte: 160 } }, { nombre: 1, puntos: 1, _id: 0 })
```
