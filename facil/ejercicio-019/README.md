# Ejercicio 019 - MongoDB Facil: ropa urbana
...
```javascript
use campus_ropa_urbana
```

```javascript
db.orders.insertOne({ nombre: 'demo-19', puntos: 133, activo: true })
```

```javascript
db.orders.find({ puntos: { $gte: 95 } }, { nombre: 1, puntos: 1, _id: 0 })
```
