# Ejercicio 025 - MongoDB Facil: animacion 3D
...
```javascript
use campus_animacion_3D
```

```javascript
db.events.insertOne({ nombre: 'demo-25', puntos: 175, activo: true })
```

```javascript
db.events.find({ puntos: { $gte: 125 } }, { nombre: 1, puntos: 1, _id: 0 })
```
