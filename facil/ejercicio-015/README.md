# Ejercicio 015 - MongoDB Facil: peliculas de miedo
...
Eliminar documentos de prueba usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
use campus_peliculas_de_miedo
```

```javascript
db.builds.insertOne({ nombre: 'demo-15', puntos: 105, activo: true })
```

```javascript
db.builds.find({ puntos: { $gte: 75 } }, { nombre: 1, puntos: 1, _id: 0 })
```
