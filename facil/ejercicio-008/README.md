# Ejercicio 008 - MongoDB Facil: mecanica de motos
...
En este ejercicio trabajaras con la coleccion `tracks`. La idea no es copiar comandos sin pensar: debes decidir que forma tendran los documentos, que campos son importantes y como validar que tus consultas responden la pregunta del negocio.

## Objetivo

Validar nombres de campos usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
1. Lee el contexto y define que datos necesita guardar la coleccion `tracks`.
...
```javascript
use campus_mecanica_de_motos
```

```javascript
db.tracks.insertOne({ nombre: 'demo-8', puntos: 56, activo: true })
```

```javascript
db.tracks.find({ puntos: { $gte: 40 } }, { nombre: 1, puntos: 1, _id: 0 })
```
