# Ejercicio 001 - MongoDB Facil: esports shooter
...
Crear una base de datos y una coleccion usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
use campus_esports_shooter
```

```javascript
db.players.insertOne({ nombre: 'demo-1', puntos: 7, activo: true })
```

```javascript
db.players.find({ puntos: { $gte: 5 } }, { nombre: 1, puntos: 1, _id: 0 })
```
