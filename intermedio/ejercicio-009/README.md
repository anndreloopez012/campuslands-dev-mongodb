# Ejercicio 009 - MongoDB Intermedio: autos de lujo
...
Preparar reportes de coleccion usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.sessions.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.sessions.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
