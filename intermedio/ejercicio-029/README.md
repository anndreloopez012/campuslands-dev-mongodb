# Ejercicio 029 - MongoDB Intermedio: esports shooter
...
```javascript
db.teams.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.teams.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
