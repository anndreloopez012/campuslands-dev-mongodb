# Ejercicio 033 - MongoDB Intermedio: futbol
...
```javascript
db.inventory.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.inventory.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
