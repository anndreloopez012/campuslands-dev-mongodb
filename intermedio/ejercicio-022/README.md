# Ejercicio 022 - MongoDB Intermedio: paracaidismo
...
```javascript
db.movies.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.movies.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
