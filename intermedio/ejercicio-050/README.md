# Ejercicio 050 - MongoDB Intermedio: paracaidismo
...
```javascript
db.vehicles.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.vehicles.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
