# Ejercicio 036 - MongoDB Intermedio: mecanica de motos
...
```javascript
db.routes.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.routes.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
