# Ejercicio 026 - MongoDB Intermedio: arquitectura 3D
...
Validar esquemas con jsonSchema usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.products.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.products.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
