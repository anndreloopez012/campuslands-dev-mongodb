# Ejercicio 032 - MongoDB Intermedio: battle royale
...
Crear indices para consultas frecuentes usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.matches.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.matches.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
