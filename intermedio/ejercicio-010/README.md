# Ejercicio 010 - MongoDB Intermedio: hiperdeportivos
...
Explicar decisiones de modelado usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.events.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.events.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
