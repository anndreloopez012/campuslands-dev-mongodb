# Ejercicio 001 - MongoDB Intermedio: esports shooter
...
Disenar documentos embebidos y referenciados usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.players.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.players.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
