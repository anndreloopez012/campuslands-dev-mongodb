# Ejercicio 023 - MongoDB Intermedio: tatuajes
...
Construir pipelines de aggregation usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.tracks.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.tracks.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
