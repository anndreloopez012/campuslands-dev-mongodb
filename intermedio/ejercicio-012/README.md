# Ejercicio 012 - MongoDB Intermedio: carreras
...
```javascript
db.bookings.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.bookings.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
