# Ejercicio 015 - MongoDB Intermedio: peliculas de miedo
...
Calcular metricas con $project usando MongoDB y dejando evidencia clara de los comandos ejecutados.
...
```javascript
db.builds.createIndex({ categoria: 1, puntos: -1 })
```

```javascript
db.builds.aggregate([{ $match: { activo: true } }, { $group: { _id: '$categoria', total: { $sum: '$puntos' }, promedio: { $avg: '$puntos' } } }, { $sort: { total: -1 } }])
```
